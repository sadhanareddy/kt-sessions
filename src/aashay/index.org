...
** ELK Videos 
   ...
   + Create an index in elasticsearch and then in view the same in kibana
   ...
   + Search filter in kibana dashboard 
   + Backup of analytics in elasticsearch 
...
** Open edX theme videos 
   ...
   + Open edX Landing page (theme components)
   + How we are getting the usage data on https://vlabs.ac.in ?
   + Backing up VLABS Open edX AWS instance 
   + Theming in College Cloud
   
** Open edX Analytics server 
   + Introduction to analytics api server
   + Introduction to various analytics APIs
